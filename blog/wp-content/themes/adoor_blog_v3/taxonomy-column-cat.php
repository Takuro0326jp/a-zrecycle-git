<?php get_header(); $current_term = single_term_title("", false); ?>

  <div id="blog-archive" class="column-archive">
    <h2><span><?php echo $current_term; ?>のコラム一覧</span></h2>
    <?php if ( have_posts() ): ?>
  ...
  <?php
  $term_slug = get_query_var('column-cat');
  $term = get_term_by( 'slug', $term_slug, 'column-cat');
  ...
  $args = array(
    ...
    query_posts($args);
    if ( have_posts() ) : ?>
    <div id="faq-main" class="column-faq">
      <h3 class="heading-3"><span><?php echo $current_term; ?>に関する<br>よくあるご質問</span></h3>
      <ul class="faq-list">
        <?php while (have_posts()) : the_post(); ?>
        <li class="clearfix">
          <dl>
      ...
      <div class="btn-area">
        <a href="/faq<?php echo '/?column_cat&#91;&#93;='.$term_id; ?>">もっと見る</a>
      </div>
    </div>
  <?php endif; wp_reset_query(); ?>
